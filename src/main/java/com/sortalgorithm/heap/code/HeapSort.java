...
public class HeapSort {
    ...
    static void heapify(int[] arr, int parentIdx, int lastIdx) {
        int leftChildIdx, rightChildIdx, largeValueIdx;

        while ((parentIdx * 2) + 1 <= lastIdx) {
            leftChildIdx = (parentIdx * 2) + 1;
            rightChildIdx = (parentIdx * 2) + 2;
            largeValueIdx = parentIdx;

            if (arr[leftChildIdx] > arr[largeValueIdx]) {
                largeValueIdx = leftChildIdx;
            }

            // 오른쪽 노드의 유무를 판단하기 위해 마지막 인덱스와 비교하는 조건이 추가되었다.
            if (rightChildIdx <= lastIdx && arr[rightChildIdx] > arr[largeValueIdx]) {
                largeValueIdx = rightChildIdx;
            }

            // 미리 선언해 부모 노드의 인덱스로 초기화해둔 largeValueIdx 이 parentIdx 와 같지 않다면,
            // 왼쪽 또는 오른쪽 자식이 더 크다는 의미이므로, SWAP 수행 후 자식 노드의 서브 트리가
            // 최대 힙의 조건을 만족하는 지 확인하기 위해, parentIdx를 자식 노드의 인덱스로 변경해준다.
            if (largeValueIdx != parentIdx) {
                int tmp = arr[parentIdx];
                arr[parentIdx] = arr[largeValueIdx];
                arr[largeValueIdx] = tmp;

                parentIdx = largeValueIdx;
            } else {
                return;
            }
        }
